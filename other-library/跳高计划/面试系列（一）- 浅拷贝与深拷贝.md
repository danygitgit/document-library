[面试系列（一）- 浅拷贝与深拷贝 | 掘金技术征文](https://github.com/danygitgit/Cheer-for-yourself/blob/master/myNotes/%E5%B5%97%E6%9C%88/%E6%88%91%E7%9A%842020/2020%EF%BC%8C%E6%AD%A5%E5%B1%A5%E4%B8%8D%E5%81%9C.md)
===
> create by **db** on **2020-3-26 19:35:50**   
> Recently revised in **2020-3-26 19:35:55**
> 
**闲时要有吃紧的心思，忙时要有悠闲的趣味**

| <a name="chapter-menu" id="chapter-menu">目录</a> |
| --- | 
| <a name="catalog-chapter-one" id="catalog-chapter-one"></a>[前言](#chapter-one) |
| <a name="catalog-chapter-three" id="catalog-chapter-three"></a>[二、浅拷贝和深拷贝初探](#chapter-three) |
| <a name="catalog-chapter-four" id="catalog-chapter-four"></a>[三、浅拷贝的实现](#chapter-four) |
...
| <a name="catalog-chapter-five" id="catalog-chapter-five"></a>[四、 深拷贝](#chapter-five) |
...
| <a name="catalog-chapter-six" id="catalog-chapter-six"></a>[总结](#chapter-six) |

# <a name="chapter-one" id="chapter-one">前言</a>
...
&emsp;下面我们就开始准备我们的面试必考题——浅拷贝与深拷贝。

# <a name="chapter-two" id="chapter-two">正文</a>

## <a name="chapter-two-one" id="chapter-two-one">一、JavaScript的数据类型
</a>
...
JavaScript的数据类型分为`值类型`和`引用数据类型`。
...
> 引用类型的值是对象，保存在堆内存中，而栈内存保存的对象的变量标识符和对象存储在堆内存中的存储地址
...
&emsp;如上，由于`基本数据类型`是直接存储的，所以如果我们对基本数据类型进行拷贝，然后修改新数据后，**不会影响到原数据**。

&emsp;而当你对`引用数据类型`进行拷贝，然后修改新数据后，它就会**影响到原数据**。

## <a name="chapter-three" id="chapter-three">二、浅拷贝和深拷贝初探</a>

&emsp;OK，了解 **基本数据类型和引用数据类型** 后，咱们进一步探索赋值、浅拷贝和深拷贝：
...
## <a name="chapter-four" id="chapter-four">三、浅拷贝的实现</a>

**浅拷贝的目标**

&emsp;被复制对象的所有变量都含有与原来的对象相同的值，而所有的对其他对象的引用仍然指向原来的对象。即对象的浅拷贝会对“主”对象进行拷贝，但不会复制主对象里面的对象。`里面的对象`会在原来的对象和它的副本之间共享。
...
## <a name="chapter-five" id="chapter-five">四、 深拷贝</a>
...
# <a name="chapter-six" id="chapter-six">总结</a>
